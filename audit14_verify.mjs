...
const res = await pool.query(`SELECT number, "serviceName", competitors, "whyNow" FROM "Idea" WHERE number IN (142, 144, 146, 149) ORDER BY number`);
for (const r of res.rows) {
  console.log(`\n=== #${r.number} ${r.serviceName} ===`);
  console.log('competitors:', r.competitors);
  console.log('whyNow:', r.whyNow?.substring(0, 300));
}
await pool.end();
