...
\`\`\`json
{
  ...
  "whyNow": "なぜ今このビジネスが成立するのか、3〜4文の文章で説明する。規制変化・技術転換点・行動変容のうち該当するものを具体的に引用し、「〇〇年の××法改正により」「□□市場が年率△%で成長しており」など数値や事実を文中に織り込む。箇条書き不可。",

  "noveltyNote": "この事業の新規性を3〜4文の文章で説明する。問題の新規性（この課題が今なぜ顕在化したか）・解法の新規性（既存手段と何が根本的に違うか）・タイミングの新規性（なぜ3年前は成立せず今は違うか）を一続きの文章として説明する。箇条書き不可。",

  "patternRationale": "選択したパターンをなぜこの組み合わせにしたか。各パターンが事業のどの部分を担い、互いにどう補完・増幅するかを2〜3文で説明する。"
}
\`\`\``;
