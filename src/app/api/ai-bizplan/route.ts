...
// Extract web search result URLs from message for source verification
...
export async function POST(request: Request) {
  try {
    ...
    if (customIdeaId) {
      ...
      ideaData = JSON.parse(custom.result);
    } else if (ideaId) {
...
以下のJSON形式のみを返してください:
{
  ...
  "factCheckNotes": ["リサーチで確認できなかった点や推計値の注記"]
    ...
    return NextResponse.json({ id: bizPlan.id, ...plan });
  } catch (error: any) {
